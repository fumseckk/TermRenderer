...
void sigwinch_handler(int sig) { update_winsize(); }
...
void init_window() {
  FATAL_ERROR(tcgetattr(STDIN, &term_settings) < 0,
              "Could not retrieve terminal settings for stdin");
  ...
  FATAL_ERROR(tcsetattr(STDIN, 0, &term_settings) < 0,
              "Could not set terminal settings for stdin");
  ...
  FATAL_ERROR(
      signal(SIGWINCH, sigwinch_handler) == SIG_ERR ||
...
void leave_window() {
  FATAL_ERROR(tcsetattr(STDIN, 0, &term_settings) < 0,
              "[CNSR] Could not set config for stdin");
  free_buff();
  fflush(stdout);
  initialized = false;
  CLEAR_SCREEN();
...
void wait_and_leave_window() {
  // Waits for SIGINT signal, which is handled using signal function. SIGINT
  // will call the leave_window function.
  while (1) sleep(1);
}
