...
Video* read_vid(char* video_path, char* frames_path, unsigned int framerate, char* start_pos, char* end_pos) {
    ...
    system(buff);
    sprintf(buff, 
        "ffmpeg -i %s -ss %s -to %s -vf scale=w=%d:h=%d:force_original_aspect_ratio=decrease -r %d %sframe%%04d.ppm -hide_banner -loglevel error", 
        video_path, start_pos , end_pos, width, height, framerate, frames_path); 
    system(buff);
    ...
    for (int i = 1; i <= frames_number; i++) {
        sprintf(buff, "%sframe%04d.ppm", frames_path, i); // Creates path for each frame (path = ./frames_path/frame[i].ppm)
